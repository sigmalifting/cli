...
const getProgramCustomLiftKeys = (program) =>
  new Set(
    (program.custom_anchored_lifts || [])
      .map(normalizeLiftKey)
      .filter(Boolean)
  );

const getCustomLiftRenameMap = (previousProgram, nextProgram) => {
  if (!previousProgram) {
    return new Map();
  }

  const previousKeys = getProgramCustomLiftKeys(previousProgram);
  const nextKeys = getProgramCustomLiftKeys(nextProgram);
  const removed = [...previousKeys].filter((key) => !nextKeys.has(key));
  const added = [...nextKeys].filter((key) => !previousKeys.has(key));

  if (removed.length === 1 && added.length === 1) {
    return new Map([[removed[0], added[0]]]);
  }

  return new Map();
};

const syncOneRmEntry = (entry = {}, customLiftKeys, renameMap) => {
  const synced = { ...entry };

  renameMap.forEach((newKey, oldKey) => {
    if (synced[newKey] === undefined && synced[oldKey] !== undefined) {
      synced[newKey] = synced[oldKey];
    }
  });

  const allowedKeys = new Set([...STANDARD_ONE_RM_LIFTS, ...customLiftKeys]);
...
const syncOneRmProfileWithProgram = (
  profile = {},
  program,
  previousProgram = null
) => {
  const customLiftKeys = getProgramCustomLiftKeys(program);
  const renameMap = getCustomLiftRenameMap(previousProgram, program);
  const synced = { ...profile };
  ...
  synced.enable_blockly_one_rm = Boolean(synced.enable_blockly_one_rm);
  STANDARD_ONE_RM_LIFTS.forEach((key) => {
    if (synced[key] === undefined) {
      synced[key] = -1;
    }
  });
  customLiftKeys.forEach((key) => {
    if (synced[key] === undefined) {
      synced[key] = -1;
    }
  });

  const existingBlocks = Array.isArray(profile.blockly_one_rm)
    ? profile.blockly_one_rm
    : [];
  synced.blockly_one_rm = program.blocks.map((_, blockIndex) =>
    syncOneRmEntry(existingBlocks[blockIndex] || {}, customLiftKeys, renameMap)
  );

  return synced;
};

const syncExerciseRecordingsWithProgram = (recordings = [], program, exercises) => {
  const warnings = [];
  const existingByExerciseId = new Map();
  const existingRecordingIndexes = new Map();

  recordings.forEach((recording, index) => {
    existingByExerciseId.set(recording.exercise_id, recording);
    existingRecordingIndexes.set(recording.exercise_id, index);
  });

  const blockDurations = new Map(
    (program.blocks || []).map((block) => [block._id, block.duration || 1])
  );
  const currentExerciseIds = new Set(exercises.map((exercise) => exercise._id));

  recordings.forEach((recording, index) => {
    if (!currentExerciseIds.has(recording.exercise_id)) {
      warnings.push({
        path: `process.exercise_recordings[${index}]`,
        message: `Removed recording for exercise ${recording.exercise_id} because it no longer exists in the linked program`,
        code: "removed_deleted_exercise_recording",
      });
    }
  });

  const syncedRecordings = exercises.map((exercise) => {
    const existing = existingByExerciseId.get(exercise._id) || {
      exercise_id: exercise._id,
      weekly: [],
    };
    const duration = blockDurations.get(exercise.block_id) || 1;
    const existingWeekly = Array.isArray(existing.weekly) ? existing.weekly : [];
    const targetLength = Math.max(duration, existingWeekly.length);

    if (existingWeekly.length > duration) {
      warnings.push({
        path: `process.exercise_recordings[${existingRecordingIndexes.get(exercise._id)}].weekly`,
        message: `Retained ${existingWeekly.length - duration} out-of-plan week(s) for exercise ${exercise._id} after linked program duration changed`,
        code: "retained_out_of_plan_weeks",
      });
    }

    const weekly = Array.from({ length: targetLength }, (_, weekIndex) => {
      const existingWeek = existing.weekly?.[weekIndex];
      return existingWeek
        ? {
            sets: Array.isArray(existingWeek.sets) ? existingWeek.sets : [],
            note: typeof existingWeek.note === "string" ? existingWeek.note : "",
          }
        : { sets: [], note: "" };
    });
...
const syncProcessBundleWithProgram = (
  processBundle,
  programBundle,
  previousProgram = null
) => {
  const synced = deepClone(processBundle);
  synced.program = deepClone(programBundle.program);
  synced.exercises = deepClone(programBundle.exercises);
  synced.process.program_id = programBundle.program._id;
  synced.process.program_name = programBundle.program.name;
  synced.process.updated_at = nowIso();
  synced.process.one_rm_profile = syncOneRmProfileWithProgram(
    synced.process.one_rm_profile || {},
    programBundle.program,
    previousProgram
  );
  const recordingSync = syncExerciseRecordingsWithProgram(
    synced.process.exercise_recordings || [],
    programBundle.program,
    programBundle.exercises
  );
  synced.process.exercise_recordings = recordingSync.recordings;

  return {
    bundle: synced,
    warnings: recordingSync.warnings,
  };
};
...
const loadStoredEntityIfExists = async (paths, entityType, id) => {
  try {
    ...
    if (String(error.message || "").startsWith(`No stored ${entityType}`)) {
      return null;
    }
    throw error;
  }
...
const cascadeProgramToStoredProcesses = async (
  paths,
  programBundle,
  previousProgram = null
) => {
  const files = await listJsonFiles(paths.processes_dir);
  const updated = [];
  const errors = [];
  ...
  for (const filePath of files) {
    try {
      ...
      if (
        normalized.entity_type !== "process" ||
        normalized.bundle.process.program_id !== programBundle.program._id
      ) {
        continue;
      }

      const syncedResult = syncProcessBundleWithProgram(
        normalized.bundle,
        programBundle,
        previousProgram
      );
      const synced = normalizeBundleForStore(syncedResult.bundle);
      await writeJsonAtomic(filePath, synced.bundle);
      updated.push({
        id: synced.bundle.process._id,
        name: synced.bundle.process.name,
        path: filePath,
        relative_path: path.relative(paths.root, filePath),
        warnings: [...(syncedResult.warnings || []), ...synced.warnings],
      });
    } catch (error) {
      errors.push({
        path: filePath,
...
export const saveBundleToStore = async (bundle, { root } = {}) => {
  ...
  const previousStored =
    normalized.entity_type === "program"
      ? await loadStoredEntityIfExists(paths, "program", identity.id)
      : null;
  ...
  const cascade =
    normalized.entity_type === "program"
      ? await cascadeProgramToStoredProcesses(
          paths,
          normalized.bundle,
          previousStored?.bundle?.program || null
        )
      : { updated: [], errors: [] };
  ...
  return {
    ...
    name: identity.name,
    warnings: normalized.warnings,
    cascaded_processes: cascade.updated,
